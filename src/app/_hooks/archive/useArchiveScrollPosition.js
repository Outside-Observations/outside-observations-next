...
export const ARCHIVE_SCROLL_TOP_KEY = 'archive_scroll_top';
  ...
  try {
    return (
      (sessionStorage.getItem(ARCHIVE_SCROLL_PERCENTAGE_KEY) !== null ||
        sessionStorage.getItem(ARCHIVE_SCROLL_TOP_KEY) !== null) &&
...
export function hasSavedScrollForView(view) {
  try {
    ...
    return (pctStr !== null || topStr !== null) && viewKey === view;
  } catch {
    return false;
  }
}
...
export function applyArchiveScrollSync(view) {
  ...
  try {
    ...
    const topStr = sessionStorage.getItem(ARCHIVE_SCROLL_TOP_KEY);
    const viewKey = sessionStorage.getItem(ARCHIVE_SCROLL_VIEW_KEY);
    const hasSaved = (pctStr !== null || topStr !== null) && viewKey !== null;
    ...
    if (hasSaved) {
      const top = topStr !== null ? parseFloat(topStr) : Number.NaN;
      const hasValidTop = Number.isFinite(top) && top >= 0;
      if (hasValidTop) {
        if (maxScroll > 0) {
        ...
        if (Number.isNaN(pct) || pct < 0 || pct > 1) {
          ...
          sessionStorage.removeItem(ARCHIVE_SCROLL_TOP_KEY);
...
export function syncArchiveScrollTopFromSession(view) {
  ...
  try {
    ...
    const topStr = sessionStorage.getItem(ARCHIVE_SCROLL_TOP_KEY);
    const viewKey = sessionStorage.getItem(ARCHIVE_SCROLL_VIEW_KEY);
    if ((pctStr === null && topStr === null) || viewKey === null || viewKey !== view) {
      return;
    }
...
/** Persist current scroll as 0–1 and absolute scrollTop for this view. */
export function saveArchiveScrollPosition(view) {
  ...
  try {
    ...
    const scrollTop = Math.max(0, scrollElement.scrollTop || 0);
    const maxScroll = scrollElement.scrollHeight - scrollElement.clientHeight;
    const scrollPercentage = maxScroll > 0 ? scrollTop / maxScroll : 0;

    sessionStorage.setItem(ARCHIVE_SCROLL_PERCENTAGE_KEY, String(scrollPercentage));
    sessionStorage.setItem(ARCHIVE_SCROLL_TOP_KEY, String(scrollTop));
  ...
  useLayoutEffect(() => {
    ...
    const tick = () => {
      ...
      if (ready) {
        ...
        const revealAfterFinalSync = () => {
          setVisible(true);
        };

        if (hasSavedScrollForView(view)) {
          // Finalize restoration before reveal so users never see an intermediate position.
          requestAnimationFrame(() => {
            syncArchiveScrollTopFromSession(view);
            requestAnimationFrame(() => {
              syncArchiveScrollTopFromSession(view);
              revealAfterFinalSync();
            });
          });
          return;
        }

        // No saved position: keep hard-pinned to top before reveal.
        const forceTop = () => {
          const el = getArchiveScrollElement(view);
          if (el && el.scrollTop !== 0) {
            el.scrollTop = 0;
